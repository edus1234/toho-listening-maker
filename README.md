...
### 🎉 最新機能 (v2.4.9 - 2025-11-14)
1. **再生/停止トグル機能** 🎵: フォルダ内の音声再生ボタンに停止機能を追加
   - 再生中のボタンをクリックすると停止できます
   - ボタンの色とアイコンが変化: 青色▶️（再生） → 赤色⏹️（停止）
   - 複数の音声がある場合、1つだけ再生される（他の音声は自動停止）
   - 音声が終了すると自動的にボタンがリセットされます

### 🎉 以前の機能 (v2.4.8 - 2025-11-14)
1. **音声保存の破損修正** 🐛: 保存時に音声データが破損する重大なバグを修正
   - 以前：JSONレスポンスが誤って音声として保存されていた
   - 修正後：Web Audio APIで正しくWAVファイルに変換して保存
2. **音声再生とQRダウンロード修正** 🐛: フォルダ内のテスト再生とQRコードダウンロードが正常に動作
3. **エラーハンドリング強化** 🔧: 破損した音声データを検出して適切なエラーメッセージを表示

### 🎉 以前の機能 (v2.4 - 2025-11-12)
...
### 本番環境
- **ステータス**: ✅ 稼働中
- **最終更新**: 2025-11-14 (v2.4.9 - 再生/停止トグル機能)
- **URL**: **https://toho-listening-maker.pages.dev** (メインドメイン)
- **最新デプロイ**: https://23d56cc4.toho-listening-maker.pages.dev
...
### v2.4.9 - 再生/停止トグル機能追加 (2025-11-14) 🎵
- ✅ **再生/停止トグルボタン**: 
  - フォルダ内の音声再生ボタンをクリックして停止できるようになりました
  - ボタンの見た目が変化: 青色の再生ボタン → 赤色の停止ボタン
  - アイコンも変化: ▶️ → ⏹️
- ✅ **複数音声の自動管理**: 
  - 新しい音声を再生すると、以前の音声が自動的に停止
  - 1つの音声のみ再生される
- ✅ **自動リセット**: 
  - 音声が最後まで再生されると、ボタンが自動的に再生状態にリセット

### v2.4.8 - 音声保存の破損修正 (2025-11-14) 🚨 **重要な修正**
- ✅ **音声データ破損の修正**: 
  - **問題**: フォルダに保存した音声が `data:application/json;base64,...` という破損した形式で保存されていた
  - **原因**: `/api/merge-audio` がJSON応答を返すのに、フロントエンドが blob として受け取っていた
  - **修正**: Web Audio APIを使用して音声セグメントを正しくWAVファイルに変換して保存
- ✅ **バックエンドのコンテンツタイプ検出**: 
  - `audio/wav` と `audio/mpeg` の両方に対応
  - 破損した `application/json` データを検出してエラーを返す
- ✅ **QRコードダウンロード改善**: 
  - エラーハンドリングを強化
  - 詳細なログ出力を追加
- ⚠️ **既存データへの影響**: 破損していた本番データ（ID 2）を削除しました

### v2.4.6 - フォルダ再生と戻るボタン修正 (2025-11-14) 🐛
- ✅ **フォルダ内テスト再生修正**: 
  - `/api/tests/:id/audio` エンドポイントの `Content-Disposition` を `attachment` から `inline` に変更
  - フォルダに保存されたテストの音声が正常に再生されるようになりました
- ✅ **「スクリプト編集に戻る」ボタン修正**: 
  - `backToScriptButton` のイベントハンドラーを修正
  - 存在しない `scriptResult` 画面への遷移を `review` 画面に変更
  - 音声編集画面からスクリプト確認画面に正常に戻れるようになりました
...
- ✅ **詳細なエラーログ**: 
  - Google TTS APIのエラーレスポンスを詳細にログ出力
  - HTTPステータスコード、エラーメッセージを記録
  - デバッグが容易になる
- ✅ **ユーザーフレンドリーなエラーメッセージ**: 
  - `無料枠を使い切りました` → 新しいAPIキーの必要性を説明
  - `レート制限に達しました` → 時間をおいて再試行するよう案内
  - HTTPステータスコードを含む詳細なエラーメッセージ
- ✅ **TTS API制限の文書化**: 
  - README に API制限に関する詳細を追加
  - 無料枠: 月100万文字（約1万分の音声）
  - レート制限: 連続リクエストに注意
- ✅ **コミット**: `48888b2`

### v2.4.2 - 無制限ブランク長対応 (2025-11-12) 🚀
- ✅ **無制限のブランク長**: 
  - 最大60秒までのブランクを設定可能（以前は3秒まで）
  - 3秒のサイレンスを複数回連結することで実現
  - 例: 10秒 = 3s + 3s + 3s + 1s
- ✅ **サイレンス結合アルゴリズム**: 
  - `getSilenceBase64`関数で自動的に必要な長さのサイレンスを生成
  - 効率的なBase64連結により、任意の長さに対応
- ✅ **UIの最大値を60秒に拡張**: 
  - ブランク入力フィールドの`max`属性を60に変更
  - 長時間のブランクが必要な場合にも対応
- ✅ **コミット**: `9a53a4f`
...
- ❌ **日本語なまり英語** (削除): 
  - Google TTSは日本語音声で英語テキストを読むことに対応していないため削除
  - 技術的制限により実装不可
- ✅ **コミット**: `8f13cb0`, `42510ab`
